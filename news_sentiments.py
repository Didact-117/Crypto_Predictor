...
df = pd.DataFrame(columns=['Type','Title','DatePublished'])#initialize empty data frame with columns tpye title and date published

for i in range(len(btc_json)):#look through jsons and append to data frame news and news sentiments for all the coins 
    df = df.append({'Type':'Bitcoin','Title':btc_json[i]['title'],'Description':btc_json[i]['description'],'Body':btc_json[i]['body']
                   ,'DatePublished':btc_json[i]['datePublished'][:btc_json[i]['datePublished'].index("T")],'Polarity_of_Title':TextBlob(btc_json[i]['title']).sentiment.polarity,
                    ...
                    'Polarity_of_Body':TextBlob(btc_json[i]['body']).sentiment.polarity},ignore_index=True)#Textblbo identifies sentiment from any string in terms of polarity
...
df.to_csv('csv/news.csv',index=False)#export to csv 
...
gb = df.groupby(['Type','DatePublished']).mean()#takes mean of all news pubished in a day 
